  ...
  export default {
    ...
    methods: {
      ...
      createSVG(pathID: string, numSamples: number ,x: number, y: number, color: string) {
        ...
        logoVertices = Vertices.scale(logoVertices, 2, 2, {x: 0, y: 0});
    ...
    mounted() {
      ...
      // make sure there is nothing inside the div
      ...
      for (let i = 0; i < numObjects; i++) {
        ...
        let randomX = this.getRandomInt(Math.min(Math.max(this.getRandomInt(width - 100) + 100, 100), width - 100));
      ...
      const vBody = this.createSVG('path-v', 100, logoCenterX - 300, logoCenterY, lightColor);
      const gBody = this.createSVG('path-g', 4, logoCenterX - 100, logoCenterY, lightColor);
      const dBody = this.createSVG('path-d', 4, logoCenterX + 125, logoCenterY, lightColor);
      const cBody = this.createSVG('path-c', 4, logoCenterX + 300, logoCenterY, lightColor);
