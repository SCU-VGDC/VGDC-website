...
<script lang="ts">
  export default {
    methods: {
      ...
      createSVG(pathID: string, x: number, y: number, color: string) {
        const path = document.getElementById(pathID) as unknown as SVGPathElement;
        let logoVertices = Svg.pathToVertices(path, 2);
        logoVertices = Vertices.scale(logoVertices, 5, 5, {x: 0, y: 0});
    ...
    mounted() {
      ...
      for (let i = 0; i < numObjects; i++) {
        ...
        let randomX = this.getRandomInt(Math.min(Math.max(this.getRandomInt(width - 60), 60), width - 60));
      ...
      const logoCenterX = 500;
      const logoCenterY = 500;

      const vBody = this.createSVG('path-v', logoCenterX - 200, logoCenterY, lightColor);
      const gBody = this.createSVG('path-g', logoCenterX - 100, logoCenterY, lightColor);
      const dBody = this.createSVG('path-d', logoCenterX, logoCenterY, lightColor);
      const cBody = this.createSVG('path-c', logoCenterX - 300, logoCenterY, lightColor);

      // add all of the bodies to the world
      Composite.add(engine.world, [...objects, bottomBound, rightBound, topBound, leftBound]);
